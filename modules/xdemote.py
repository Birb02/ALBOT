...
async def run(ctx, bot, allowed, debug_log):
    ...
    if not ctx.mentions and not hierarchy_config:
        ...
        for role_data in role_index:
            index_text += f"{role_data['index']}|{role_data['name']}\n"

        await ctx.channel.send(f"```{index_text}```")
...
{",".join(hierarchy_ids)}"""
    ...
    if user_role_index is None:
        return await ctx.channel.send(f"[!] {user} does not have a role in the hierarchy")
